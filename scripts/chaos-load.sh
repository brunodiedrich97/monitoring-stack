...
echo -e "\n${GREEN}[2/4] $(timestamp) Simulando Sobrecarga de CPU por 5 minutos...${NC}"
docker run -d --rm --name staging_cpu_stress alpine sh -c '
  ...
  sleep 300
' > /dev/null
echo -e "${YELLOW}$(timestamp) Estresse de CPU rodando (container: staging_cpu_stress)${NC}"

echo -e "\n${GREEN}[3/4] $(timestamp) Ataque de Requisicoes via Nginx (porta 80) — 3 rodadas...${NC}"
...
echo -e "\n${RED}$(timestamp) [CAOS FINALIZADO]${NC} Verifique os graficos no Grafana e alertas no Slack!"
