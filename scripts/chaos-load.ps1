...
Write-Host "[1/4] Simulando Sobrecarga de CPU por 5 minutos via Docker..." -ForegroundColor Green
try {
    ...
    Write-Host "  Estresse de CPU rodando (container: staging_cpu_stress)" -ForegroundColor Yellow
} catch {
    Write-Host "  Docker não disponível — pulando estresse de CPU." -ForegroundColor Red
}

Write-Host "[2/4] Ataque de Requisicoes via Nginx (porta 80) — 3 rodadas..." -ForegroundColor Green
for ($rodada = 1; $rodada -le 3; $rodada++) {
    ...
    Write-Host "  Rodada $rodada concluída, aguardando 30s..." -ForegroundColor Yellow
    if ($rodada -lt 3) {
        Start-Sleep -Seconds 30
    }
}

Write-Host "[3/4] Varredura Maliciosa — 1500 requisicoes a rotas invalidas..." -ForegroundColor Green
...
Write-Host "  Varredura concluída (1500 reqs para rota inválida)." -ForegroundColor Yellow
